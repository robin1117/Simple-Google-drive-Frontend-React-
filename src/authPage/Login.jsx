...
export default function Login() {
  ...
  async function handleSubmit(e) {
    try {
      e.preventDefault();
      setIsLoading(true);
      const response = await sendingLoginData({ email: username, password });
  ...
  return (
    <>
      {isLoading && (
        <div className="backdrop-blur-[2px] inset-0 absolute flex items-center justify-center">
          <GridLoader
            color={"gray"}
            loading={isLoading}
            size={50}
